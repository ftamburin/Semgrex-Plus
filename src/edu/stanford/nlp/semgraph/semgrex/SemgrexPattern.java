...
public abstract class SemgrexPattern implements Serializable {
  ...
  abstract String mylocalString(boolean firstItem, boolean printDesc);
  ...
  private void myprettyPrint(boolean printDesc) {
	String first = mylocalString(true, printDesc);
	List<SemgrexPattern> childs = getChildren();
	if ((childs.size() == 1) && 
		(childs.get(0).mylocalString(false, printDesc) == "COORD"))
		childs = childs.get(0).getChildren();
    for (SemgrexPattern child : childs) {
      System.out.println(first + " " + 
						 child.mylocalString(false, printDesc));
	  child.myprettyPrint(printDesc);
    }
  }
  public void myPrettyPrint(boolean printDesc) {
	List<SemgrexPattern> childs = getChildren();
	if (childs.size() == 0)
		System.out.println(mylocalString(true, printDesc));
	else
		myprettyPrint(printDesc);
  }
  ...
  static final String PATTERNTO = "-pattern_to";
  static final String PATTERNSF = "-patternsF";
  ...
  public static void help() {
    ...
    System.err.println(PATTERNTO + ": the pattern for transformation");
    ...
    System.err.println();
    System.err.println(PATTERN + " and " + PATTERNTO + " are required");
  }
  ...
  public static void main(String[] args) throws IOException {
    ...
    // allow patterns to be extracted from a file
	File file = new File(argsMap.get(PATTERNSF)[0]);
    FileReader FIreader = new FileReader(file);
    BufferedReader buffReader = new BufferedReader(FIreader);
    String line;
	int	nrule = 0;
    while((line = buffReader.readLine()) != null){
		nrule++;
        System.err.println("Applying rule "+nrule+": "+line);

		StringTokenizer Tok = new StringTokenizer(line,"\t\n");
		String patternFrom = Tok.nextToken();
		patternFrom = patternFrom.replace("\"","");
		String patternTo = Tok.nextToken();
		patternTo = patternTo.replace("\"","");

    	SemgrexPattern semgrex = SemgrexPattern.compile(patternFrom);
		System.out.println("PATTERN_FROM");
		semgrex.myPrettyPrint(true);
		System.out.println();

    	SemgrexPattern semgrex_to = SemgrexPattern.compile(patternTo);
		System.out.println("PATTERN_TO");
		semgrex_to.myPrettyPrint(true);
		System.out.println();
    	...
    	for (SemanticGraph graph : graphs) {
      		...
      		if (outputFormat == OutputFormat.LIST) {
				...
				System.out.print("-----\ngraph" + graph.getComments().get(0) + "\n\n");
        		//System.out.println(graph.toString(SemanticGraph.OutputFormat.READABLE)); // LIST
        		boolean found = true;
        		while (found) {
          			System.out.println("Matches at: " +
									matcher.getMatch().value() + "-" + 
									matcher.getMatch().index());
          			...
          			for (String name : nodeNames) {
            			System.out.println("  " + name + ": " + 
								...
								matcher.getNode(name).tag()); 
          			}
          			System.out.println();

          			List<String> relNames = Generics.newArrayList();
          			relNames.addAll(matcher.getRelationNames());
          			Collections.sort(relNames);
          			for (String name : relNames) {
            			System.out.println("  " + name + ": " + 
									matcher.getRelnString(name));
          			}
          			System.out.println();
